...
with open(output_file, "w", encoding="utf-8") as f:
    for model in model_entries:
        ...
        sorted_entries = sorted(entries, key=lambda x: x["bert"], reverse=True)
        
        best_two = sorted_entries[:2]  
        worst_two = sorted_entries[-2:]  
        
        f.write(f"Model: {model}\n")
        
        f.write("=== Top 2 Highest BERTScore Summaries ===\n")
        for i, best in enumerate(best_two, 1):
            f.write(f"\n# {i}\n")
        ...
        f.write("=== Top 2 Lowest BERTScore Summaries ===\n")
        for i, worst in enumerate(worst_two, 1):
            f.write(f"\n# {i}\n")
